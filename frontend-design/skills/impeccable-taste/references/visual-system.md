# Visual System (impeccable-taste)

> The visual rulebook of the merged skill. Adapted from design-taste-frontend; on any
> visual question (palette, type, shape, layout, motion, theming, AI tells) THIS file
> is the authority. Content architecture and section substance live in
> [content-canon.md](content-canon.md); where the two conflict on a visual matter,
> this file wins; on a content matter, content-canon.md wins.
> Every rule below is **contextual**. None of it fires automatically. First read the brief, then pull only what fits.
> In unattended/automated runs, never ask the user anything (including the Section 0.C
> single question and the Section 11.A redesign question): state the assumption inline
> and proceed with the safer reading.
...
**NO PLACEHOLDERS, EVER (standing policy, overrides everything below):**
Every image on a shipped page is a real, resolving asset sourced per
[image-sourcing.md](image-sourcing.md). No `picsum.photos`, no TODO image comments,
no empty slots, no grey boxes, no styled stand-in panels. The page is not done
while any image slot lacks a real image.

**Priority order for visual assets:**
1. **Real photography first**, sourced via API per [image-sourcing.md](image-sourcing.md):
   Unsplash for anything with a human face and for contemporary lifestyle/interiors;
   Wikimedia Commons for real equipment, places, vehicles, workspaces; Openverse
   (licence-filtered) for breadth. Query the API, never assemble URLs by hand;
   verify every URL resolves; size at the source; credit; keep one visual register
   across the surface.
2. **Image generation second**, only for what photography cannot supply: UI
   screenshots, abstract textures, invented product renders. Authored at production
   fidelity and labeled synthetic where a visitor could mistake it for real.
3. There is no third tier. If a source is unreachable, the build fails its image
   gate and says so; it does not ship a stand-in.

**Even minimalist sites need real images.** A pure-text page is not minimalism. It is incomplete work. Even an editorial Linear-style site needs at least 2-3 real images (hero, one product/lifestyle shot, one supporting image). Source restrained B&W photography per [image-sourcing.md](image-sourcing.md) if the brief is restrained; do not skip images entirely because the dial is low.
...
* **NO generic avatars.** No SVG "egg" or Lucide user icons → use real, believable people photography (Unsplash, per [image-sourcing.md](image-sourcing.md)) or specific styling.
...
* **NO broken image links and NO placeholders.** Never hand-assemble an Unsplash (or any) photo URL from memory; IDs are opaque and hand-assembled ones 404. Query the source API and verify every URL resolves (see [image-sourcing.md](image-sourcing.md)). `picsum.photos` is banned outright.
...
* **NO photo-credit captions as decoration.** Strings like `Field study no. 12 · Ines Caetano`, `Plate 03 · House archive`, `Frame XII · 35mm` under stock images are pretentious. Photo credit is allowed ONLY when there is a real photographer being credited for a real photo (with permission). Otherwise: skip the caption or use a one-line functional caption ("The 6-quart, in Sage.").
...
The upstream skill reserved this section for a block library that was never populated; removed in the merge. Numbering is kept stable so cross-references to Sections 13 and 14 stay valid.
...
- [ ] **Real images used** (API-sourced photography per image-sourcing.md, generation only for what photography cannot supply) - ZERO placeholders (no picsum, no TODO image comments, no empty slots, no stand-in panels), every image URL verified 200 with an image content-type, licences cleared (no NC/ND), one source register across the surface - NO div-based fake screenshots, NO hand-rolled decorative SVGs, NO pure-text minimalism?
