...
1. `live.mjs`: boot. If the request names or implies a file, route, or app inside a monorepo, infer the concrete path and run `node ${CLAUDE_PLUGIN_ROOT}/skills/impeccable/scripts/live.mjs --target <path>` instead; then run the rest of this live session from the returned `projectRoot`.
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/skills/impeccable/scripts/live.mjs
```
...
```
LOOP:
  node ${CLAUDE_PLUGIN_ROOT}/skills/impeccable/scripts/live-poll.mjs   # default long timeout; no --timeout=
...
```
node ${CLAUDE_PLUGIN_ROOT}/skills/impeccable/scripts/live-poll.mjs --stream   # stays running; one JSON line per event
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/skills/impeccable/scripts/live-status.mjs
node ${CLAUDE_PLUGIN_ROOT}/skills/impeccable/scripts/live-resume.mjs --id SESSION_ID
node ${CLAUDE_PLUGIN_ROOT}/skills/impeccable/scripts/live-complete.mjs --id SESSION_ID
```
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/skills/impeccable/scripts/live-insert.mjs --id EVENT_ID --count EVENT_COUNT --position after \
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/skills/impeccable/scripts/live-wrap.mjs --id EVENT_ID --count EVENT_COUNT --element-id "ELEMENT_ID" --classes "class1,class2" --tag "div" --text "TEXT_SNIPPET"
```
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/skills/impeccable/scripts/live-poll.mjs --reply EVENT_ID done --file RELATIVE_PATH
```
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/skills/impeccable/scripts/live-poll.mjs --reply EVENT_ID error "Short reason"
```
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/skills/impeccable/scripts/live-poll.mjs --reply EVENT_ID steer_done ["Optional short note for a browser toast"]
```
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/skills/impeccable/scripts/live-server.mjs stop
```
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/skills/impeccable/scripts/detect-csp.mjs
```
