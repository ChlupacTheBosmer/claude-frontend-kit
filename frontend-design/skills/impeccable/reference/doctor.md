...
```
node ${CLAUDE_PLUGIN_ROOT}/skills/impeccable/scripts/doctor.mjs --json
```
...
- **`auto`** carries no decision. Run `node ${CLAUDE_PLUGIN_ROOT}/skills/impeccable/scripts/doctor.mjs --fix` once to apply these, then report what it moved in one line. Do not ask permission first, and do not ask about them afterward.
